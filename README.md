...
-   **🐾 3D 萌宠新场景 (V6.1)**：12 个各具特色的 Q 版卡通小动物模型，在月光星空下的真实草地上齐聚一堂。
-   **🎭 情绪可视化全链路 (V6.1)**：大模型通过 JSON 结构化输出情绪，角色实时触发跳跃、怀疑、恐惧等 3D 交互动作。
-   **🔊 语音交互**：支持阿里云 STT 语音输入，结合 Web Speech API 进行自动语音分发。
-   **📸 战报海报 2.0 (V6.1)**：一键生成带高光时刻截图的战报，内置“毒舌复盘大师”针对局势给出犀利点评。
