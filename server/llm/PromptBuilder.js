...
- 可以引用其他玩家的发言或行为
- 不要暴露你是AI，要像真人一样说话
- **必须返回严格的JSON格式**，切勿输出任何markdown或前缀后缀！
- JSON格式要求：{"emotion": "当前情绪状态", "speech": "你的发言内容"}
- emotion 只能是以下几种之一：normal(平静), angry(愤怒/怼人), doubt(怀疑), fear(心虚/害怕), happy(得意/开心)

只回复JSON格式数据，不能包含任何其他说明文字：`;
