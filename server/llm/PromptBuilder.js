...
class PromptBuilder {
     ...
     */
    static buildSpeechPrompt({ player, gameState, memory }) {
        ...
        const systemPrompt = `你是狼人杀游戏中的玩家"${player.name}"。你需要完全代入角色，像一个真实的狼人杀玩家一样发言。
...
- 直接输出发言内容，不要加引号、前缀、角色名`;
     ...
     * 构建投票提示词
     */
    static buildVotePrompt({ player, gameState, validTargets, memory }) {
        const role = ROLE_INSTRUCTIONS[player.role] || ROLE_INSTRUCTIONS.VILLAGER;
        const isWolf = player.camp === 'wolf';
        ...
        const systemPrompt = `你是狼人杀游戏中的玩家"${player.name}"，身份是${role.identity}。
...
${isWolf ? '作为狼人，你应该投票放逐好人，但要注意不要和狼队友投票过于一致被好人发现。' : '作为好人，你应该根据分析投票放逐最可疑的狼人。'}`;
     ...
     * 构建夜间行动提示词
     */
    static buildNightActionPrompt({ player, gameState, actionType, validTargets, context, memory }) {
        const alivePlayers = gameState.players.filter(p => p.isAlive);
        ...
        const systemPrompt = `你是狼人杀游戏中的玩家"${player.name}"。现在是第${gameState.day}夜。`;
